        ...
        for msg in messages:
            ...
            if isinstance(content, list):
                ...
                # Check if this is an assistant message with tool_use blocks
                    ...
                    # ✅ 修复: text_parts 为空时用空字符串而非 None，防止 str+None TypeError
                    openai_msg = {
                        "role": "assistant",
                    ...
                    # ✅ 修复: 用户普通文本块消息（无 tool_result），展平为字符串
                    # 否则 list 格式的 content 保留 Claude 格式，部分模型无法理解
                    text_parts = [
                        block.get("text", "")
                        for block in content
                        if isinstance(block, dict) and block.get("type") == "text"
                    ]
                    flat_text = " ".join(t for t in text_parts if t).strip()
                    if flat_text:
                        openai_messages.append({"role": "user", "content": flat_text})
                    else:
                        # No extractable text; keep original to avoid losing the message
                        openai_messages.append(msg)
